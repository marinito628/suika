...
if(!empty($_POST['henban'])){
	...
	foreach($results as $row[]){
		...
		//編集機能
		$id = $henban; //変更する投稿番号
		$name = $hen_komagiri[$r][1];
		$comment =$hen_komagiri[$r][2]; //変更したい名前、変更したいコメントは自分で決めること
...
if(!empty($_POST['hen_num'])){
	...
	foreach ($hen_numresults as $hen_numrow[]){
		...
		if($hen_num==$hen_numrow[$f][0] && $_POST['pass2']==$komagiri2[$f][4]){
